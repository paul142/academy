...
![local port 1234 td](Attachments/949BE55D-9905-4B79-A7C7-CEAF98BA2EC8.png)  
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ nmap -sT -p22,3306 10.129.202.64
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ ssh -L 1234:localhost:3306 ubuntu@10.129.202.64
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ netstat -antp | grep 1234
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ nmap -v -sV -p1234 localhost
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ ssh -L 1234:localhost:3306 -L 8080:localhost:80 ubuntu@10.129.202.64
...
This technique is often used to circumvent the restrictions put in place by firewalls, and allow an external entity to bypass the firewall and access a service within the firewalled environment. One more benefit of using SOCKS proxy for pivoting and forwarding data is that SOCKS proxies can pivot via creating a route to an external server from ****NAT networks****. SOCKS proxies are currently of two types: ****SOCKS4**** and ****SOCKS5****. SOCKS4 doesn't provide any authentication and UDP support, whereas SOCKS5 does provide that. Let's take an example of the below image where we have a NAT'd network of 172.16.5.0/23, which we cannot access directly.  
![on port 9050](Attachments/0EC00F32-B941-4D92-A643-5C899CBE2C8C.png)  
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ ssh -D 9050 ubuntu@10.129.202.64
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ tail -4 /etc/proxychains.conf
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ proxychains nmap -v -sn 172.16.5.1-200
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ proxychains nmap -v -Pn -sT 172.16.5.19
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ proxychains msfconsole
...
Dynamic Port Forwarding with SSH and SOCKS Tunneling  
```
Pavlo142@htb[/htb]$ proxychains xfreerdp /v:172.16.5.19 /u:victor /p:pass@123
...
![(13:38-81:839) 1.86. -90-000](Attachments/2FFB6F53-08A1-4E6F-8FA6-80FB76591624.png)  
